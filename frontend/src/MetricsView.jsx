...
function BarChart({ data, color = 'var(--teal)', label }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      {label && <div style={{ fontSize: '11px', color: 'var(--muted)', fontFamily: 'JetBrains Mono', marginBottom: '4px' }}>{label}</div>}
...
export default function MetricsView() {
  ...
  useEffect(() => {
    fetchMetrics();
    pollRef.current = setInterval(fetchMetrics, 10000); // live refresh every 10s
  ...
  return (
    <div className="fade-in-section container-responsive" style={{ paddingTop: '40px', paddingBottom: '60px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px', marginBottom: '28px' }}>
        <StatCard icon="👥" label="DAU" value={m.dau ?? '—'} sub="Unique logins today" color="var(--teal)" delay={0} />
        <StatCard icon="📅" label="WAU" value={m.wau ?? '—'} sub="Active this week" color="#818cf8" delay={0.05} />
        <StatCard icon="🌱" label="Total Users" value={m.totalUsers ?? '—'} sub={`+${m.newUsersToday ?? 0} today`} color="var(--green)" delay={0.1} />
        <StatCard icon="⚡" label="Tx Today" value={m.txCountToday ?? '—'} sub={`${m.txCountTotal ?? 0} all-time`} color="var(--gold)" delay={0.15} />
        <StatCard icon="💰" label="Total Volume" value={`₹${Number(m.totalVolume || 0).toLocaleString('en-IN')}`} sub={`₹${Number((m.txByDay ? Object.values(m.txByDay).reduce((a,b)=>a+b,0) : 0)).toLocaleString('en-IN')} this week`} color="#f472b6" delay={0.2} />
      </div>
      ...
      {/* ── Row 3: Recent Transactions + New Users split ── */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '16px' }}>
              ...
              {m.recentTxs.map((tx, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', background: 'rgba(255,255,255,0.02)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.04)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <div style={{ width: '32px', height: '32px', borderRadius: '8px', background: 'rgba(45,212,191,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px' }}>⚡</div>
                    <div>
        ...
        >
          {[
            { label: 'New Users Today', value: m.newUsersToday ?? 0, color: 'var(--green)', icon: '🌱' },
            { label: 'New Users This Week', value: m.newUsersWeek ?? 0, color: 'var(--teal)', icon: '📈' },
            { label: 'Tx This Week', value: m.txCountWeek ?? 0, color: 'var(--gold)', icon: '🔗' },
            { label: 'All-Time Tx', value: m.txCountTotal ?? 0, color: '#818cf8', icon: '📊' },
          ].map((item, i) => (
            <motion.div
              key={item.label}
